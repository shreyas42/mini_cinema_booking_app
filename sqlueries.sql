...
where f.format_id in (
	...
	where movie_title in (
		...
		where director_name in (
			...
			from director 
			where gender='F'))
	and movie_title=m.title);


#2 - Display the Daniel Craig film which screens after 18:00 on weekend less than 120 min runtime
...
#5 - Display customers who spent more than 1000 on booking tickets
...
where email in (
	...
	join seat on (
		s_row_num=row_num 
		and s_col_num=col_num) 
	group by cust_email 
	having sum(price)>1000);


#6 - Display film title, director, runtime, language, format in a view
...
#7 - Display directors who directed only English films with runtime less than 120 min
...
#8 - Display shows of an English language film on 2017/05/15 before noon
...
#9 - Display film title, director, critics score of films screened in at 09:00 in a view
...
#10 - Display the day with the most seats booked

select s.show_date
from show s 
join show_details d on (
	s.show_id=d.show_id 
	and d.s_status='B') 
